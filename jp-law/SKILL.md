...
短い英字略語（AI / IoT / DX 等）は API でヒットしにくい場合がある。次の順で試行する:

1. `bash scripts/search-keyword.sh <英字略語>` を試す
2. 0 件（`{"code":"404001"}`）の場合は日本語訳語（人工知能 / モノのインターネット / デジタルトランスフォーメーション 等）に切り替えて再試行
3. 法令そのものを探す場合は `bash scripts/search-laws.sh <日本語訳語>` も併用し、法令名レベルでマッチを確認

例: 「AI に関する法律」→ `AI`（0 件）→ `人工知能`（条文 160 件ヒット）→ `bash scripts/search-laws.sh 人工知能` で `507AC0000000053`（人工知能関連技術の研究開発及び活用の推進に関する法律、通称ＡＩ法）を発見。
