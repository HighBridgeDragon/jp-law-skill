...
**入力プロンプト**: 会社法の株式に関する章（第2編第2章）を表示して
...
- [x] 章まとめ取得のレスポンスサイズが全文取得より大幅に小さい（観測値: 章まとめ 約 420KB vs 全文 約 2,952KB → 約 7.0 倍削減）
- [x] 出力フォーマットは SKILL.md 末尾の推奨フォーマットに沿う

章単位取得のもう一つの主要な効果は、API 呼出回数の削減である。154 件の条文を条文単位（`Article_X`）で個別取得すると 154 回の API 呼出が必要になるが、章単位 `Chapter_2` で 1 回にまとめられる。レスポンスサイズの比較は全文取得との対比（420KB vs 2,952KB）で十分示せるため、本テストでは呼出回数削減を併せて評価指標とする。

### 所見

**elm 指定の修正過程**: `MainProvision-Part_3-Chapter_2` を最初に試みたところ、会社法の第三編（持分会社）第二章「社員」（第580条〜第589条、10 件）が返却され、問い合わせ意図（株式に関する章）と異なった。会社法の構造（第二編 株式会社 / 第三編 持分会社）を確認のうえ `MainProvision-Part_2-Chapter_2` に修正することで正しい章（"第二章 株式"、154 件）を取得できた。

**学び**: 章単位 elm の指定は法令の編章構造を事前に把握したうえで行う必要がある。

**サイズ削減効果**: 章まとめ取得（420KB）は全文取得（2,952KB）と比べて約 7 倍の削減効果があり、SKILL.md のトークン節約ガイダンスの有効性を実測で裏付けている。
